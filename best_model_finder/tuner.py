...
class Model_Finder:
    ...
    def get_best_params_for_random_forest(self, train_x, train_y):
        ...
        try:
            ...
            self.logger_object.log(self.file_object,
                                   'RandomForest Parameter tuning  failed. Exited the get_best_params_for_random_forest'
    ...
    def get_best_params_for_KNN(self, train_x, train_y):
        ...
        """
        self.logger_object.log(self.file_object,
                               'Entered the get_best_params_for_Ensemble_KNN method of the Model_Finder class')
        try:
            # initializing with different combination of parameters
            self.param_grid_knn = {
            ...
            self.logger_object.log(self.file_object,'Exception occured in knn method of the Model_Finder class.'
    ...
    def get_best_model(self, train_x, train_y, test_x, test_y):
        ...
        try:
            ...
            self.logger_object.log(self.file_object, 'Predictions using the KNN Model successful!!!')

            if len(test_y.unique()) == 1:  # if there is only one label in y, then roc_auc_score returns error. We
                # will use accuracy in that case
                self.logger_object.log(self.file_object, 'only one label in y')
                try:
                    df = pd.DataFrame(test_y)
                    df.to_csv('Training_FileFromDB/test_y.csv', index=False)
                    self.logger_object.log(self.file_object, 'test_y file creation  successful!!!')
                except Exception as e:
                    self.logger_object.log(self.file_object, 'Error occur :' + str(e))

                try:
                    df1 = pd.DataFrame(self.prediction_knn)
                    df1.to_csv('Training_FileFromDB/prediction_knn.csv', index=False)
                    self.logger_object.log(self.file_object, 'prediction_knn  successful!!!')
                except Exception as e:
                    self.logger_object.log(self.file_object, 'Error occur :' + str(e))
                ...
                self.logger_object.log(self.file_object, 'More than one label in y')
                try:
                    df = pd.DataFrame(test_y)
                    df.to_csv('Training_FileFromDB/test_y.csv', index=False)
                    self.logger_object.log(self.file_object, 'test_y file creation  successful!!!')
                ...
                try:
                    df1 = pd.DataFrame(self.prediction_knn)
                    df1.to_csv('Training_FileFromDB/prediction_knn.csv', index=False)
                    self.logger_object.log(self.file_object, 'prediction_knn csv file creation  successful!!!')
                except Exception as e:
                    self.logger_object.log(self.file_object, 'Error occur :' + str(e))

                self.knn_score = roc_auc_score(test_y, self.prediction_knn, multi_class='ovr')  # AUC for KNN
                # self.logger_object.log(self.file_object, 'Error occurred :'+str(e))
            ...
            self.logger_object.log(self.file_object,'Exception occured in get_best_model method of the Model_Finder class. '
                                                    'Exception message:  ' + str(e))
            self.logger_object.log(self.file_object,
