...
class dBOperation:
    """
          This class shall be used for handling all Cassandra Database operations.
    ...
    def dataBaseConnection(self):
        ...
        try:
            ...
            self.logger.log(file, " Cassandra  Prediction database connected successfully")
            file.close()
    ...
    def createTableDb(self):
        ...
        Method Name: createTableDb Description: This method creates a table in the given database which will be used
                     to insert the Good data after raw data validation. Output: None On Failure: Raise Exception
        ...
        try:
            session.execute('USE prediction')
            ...
            session.execute("CREATE TABLE Good_Raw_Data(serial_no int PRIMARY KEY, age int, sex varchar, "
                            ...
                            "TBG_measured varchar, TBG varchar, referral_source varchar);")
            ...
            file = open("Prediction_Logs/DbTableCreateLog.txt", 'a+')
            self.logger.log(file, "Error while creating Prediction table: %s " % e)
            session.shutdown()
            self.logger.log(file, "Database closed successfully")
            file.close()
            raise e
    ...
    def insertIntoTableGoodData(self):
        ...
        try:
            ...
            with open("Prediction_FileToDB/combined_csv.csv", "r") as f:
                next(f)
                ...
                for i in reader:
                    session.execute(
                        ...
                        'TBG_measured, TBG,referral_source) values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,'
                        ...
                        [int(i[0]), int(i[1]), i[2], i[3], i[4], i[5], i[6], i[7], i[8], i[9], i[10], i[11], i[12],
                         ...
                         i[26], i[27], i[28], i[29]])
            self.logger.log(log_file, 'Prediction Data inserted into cassandra DB successfully')
            session.shutdown()
    ...
    def selectingDatafromtableintocsv(self):
        ...
        try:
            ...
            query = ("SELECT serial_no,age,sex,on_thyroxine,query_on_thyroxine,on_antithyroid_medication,sick,pregnant,"
                     ...
                     "FTI_measured,FTI,TBG_measured,TBG,referral_source FROM Good_Raw_Data;")
            ...
            df.to_csv(f"{self.fileFromDb}" + '//' + f"{self.fileName}", index=False, encoding='utf-8-sig')
            ...
            self.logger.log(log_file, "Prediction File exporting failed. Error : " + str(e))
            log_file.close()
            session.shutdown()
            raise e
